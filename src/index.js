...
function showCountryFlagName(data) {
  ...
  countryDescription.classList.add('js-country-list__name');
  countryDescription.textContent = data.name.official;

  const countryFlag = document.createElement('img');
  countryFlag.classList.add('js-country-list__flag');
...
function showCountryData(obj, targetPlace) {
  ...
  for (const key of keys) {
    ...
    insertHeaderData.classList.add('js-country-list__heading');

    const insertHeaderDetails = document.createElement('span');
    insertHeaderDetails.classList.add('js-country-list__details');
...
function showUnfold(el) {
  ...
  el.classList.add('js-country-list__item--unfold');
...
document.body.addEventListener('click', e => {
  ...
  if (!e.target.classList.contains('js-country-list__item')) {
    return;
  }

  if (
    !e.target.classList.contains('js-country-list__item--unfold') &&
  ...
  if (e.target.classList.contains('js-country-list__item--unfold')) {
    e.target.classList.toggle('js-country-list__item--unfold');
